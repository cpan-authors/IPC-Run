...
sub _do_kid_and_exit {
    ...
    eval {
        ...
        if ( %{ $self->{PTYS} } ) {
            ...
            ## reopen a slave whose fd was invalidated (GH#243).
            my %ttynames;
            for ( keys %{ $self->{PTYS} } ) {
            ...
            ## GH#243: On some platforms (e.g. macOS with IO::Pty 1.21),
            ## make_slave_controlling_terminal may invalidate the cached
            ## slave fd.  Verify each slave fd is still usable; if not,
            ## reopen the slave device from its saved ttyname.
            for ( keys %{ $self->{PTYS} } ) {
