...
sub _cleanup {
    ...
    for ( values %{ $self->{PTYS} } ) {
        ...
        eval {
            ...
            close $_->slave;
            ## Prevent IO::Pty >= 1.21 DESTROY from double-closing the
            ## slave fd.  $_->slave may re-cache a handle that start()
            ## already cleared — remove it so DESTROY has nothing to close.
